...
namespace DianDianClient.Utils
{
   ...
   string pDrive,
        ...
        /// </summary>  
        /// <returns></returns>  
        public static SPrinterStatus getStatus(string PrinterName)
        {
            string path = @"win32_printer.DeviceId='" + PrinterName + "'";
            ManagementObject printer = new ManagementObject(path);
            printer.Get();
            return (SPrinterStatus)Convert.ToInt32(printer.Properties["PrinterStatus"].Value);
        }
        ...
        public static string GetPrinterStatus(string PrinterName)
            ...
            public static System.Collections.ArrayList GetPrinterList()
            {
                ...
                PRINTER_INFO_2[] Info2 = EnumPrintersByFlag(PrinterEnumFlags.PRINTER_ENUM_NETWORK | PrinterEnumFlags.PRINTER_ENUM_LOCAL | PrinterEnumFlags.PRINTER_ENUM_REMOTE);//PrinterEnumFlags.PRINTER_ENUM_LOCAL
                for (int i = 0; i < Info2.Length; i++)
                {
                    alRet.Add(Info2[i].pPrinterName);
                Console.WriteLine("打印机：{0}", Info2[i].Attributes);
                Console.WriteLine("打印机：{0}", Info2[i].pShareName);
                     Console.WriteLine("打印机：{0}", Info2[i].pComment);
                Console.WriteLine("打印机：{0}", Info2[i].pLocation);
                Console.WriteLine("打印机：{0}", Info2[i].pDevMode);
                     Console.WriteLine("打印机：{0}", Info2[i].pSepFile);
                Console.WriteLine("打印机：{0}", Info2[i].pPrintProcessor);
                Console.WriteLine("打印机：{0}", Info2[i].pDatatype);
                Console.WriteLine("打印机：{0}", Info2[i].pParameters);
                Console.WriteLine("打印机：{0}", Info2[i].pSecurityDescriptor);
                Console.WriteLine("打印机：{0}", Info2[i].Priority);
                Console.WriteLine("打印机：{0}", Info2[i].DefaultPriority);
                Console.WriteLine("打印机：{0}", Info2[i].AveragePPM);
            }
                return alRet;
            }
            #endregion 获取本地打印机列表
